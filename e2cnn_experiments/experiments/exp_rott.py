...
#labels=labels
    return confusion_matrix(targets.cpu().numpy(), predictions.cpu().numpy(), labels=labels)
...
class Experiment:
    ...
    def test(self):
        ...
        # Call rotated test evaluation
        self.evaluate_rotated_mnist()

    def evaluate_rotated_mnist(self, angles=[0, 45, 90]):
        print("\n--- Rotation Test Evaluation ---")

        class RotateFixed:
            def __init__(self, angle):
                self.angle = angle
            def __call__(self, img):
                return TF.rotate(img, self.angle, fill=0)
        ...
        for angle in angles:
            ...
            rotated_test = MNIST(root="./data", train=False, transform=transform, download=True)
            loader = DataLoader(rotated_test, batch_size=self.batch_size, shuffle=False)
            ...
            acc = correct / total
            results[angle] = acc
            print(f"Test Accuracy at {angle}°: {acc:.4f}")
        print("--- End Rotation Eval ---\n")
        return results
