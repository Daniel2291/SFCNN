...
def build_mnist_rot_loader(mode, batch_size, num_workers=8, rot_interpol_augmentation=False, interpolation=0, reshuffle_seed=None, coords=False, limit_samples = None):
    """  """
    ...
    if limit_samples is not None and mode == 'train':
       rng = np.random.RandomState(reshuffle_seed or 0)
       subset_indices = rng.permutation(len(dataset))[:limit_samples]
